    ...
    <main className="relative min-h-dvh overflow-hidden antialiased">
        ...
        <GlassCard className="p-6 sm:p-10">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl/tight font-semibold tracking-tight sm:text-4xl md:text-5xl">
                Ålesund Studentsamfunn (ÅSS)
              </h1>
              <p className="mt-3 max-w-2xl text-sm/relaxed sm:text-base">
                Ålesund Studentsamfunn er en frivillig organisasjon drevet av studenter for å tilrettelegge
                for den utenomfaglige og sosiale arenaen for alle studenter i Ålesund. Vi drifter <span className="font-medium">Samfunnet Studenthus</span>
                , fyller huset med aktiviteter og koordinerer undergrupper innen alt fra dykking og klatring til musikk og spill.
              </p>

              <div className="mt-6 flex flex-wrap gap-3">
                <a
                  href="/dashboard"
                  className="group inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2 text-sm font-medium  backdrop-blur-md ring-1 ring-white/15 transition hover:bg-white/15"
                >
                  Logg inn som frivillig <ChevronRight className="h-4 w-4 transition group-hover:translate-x-0.5" />
                </a>
                <a
                  href="https://www.astudent.no" target="_blank" rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-2xl bg-transparent px-4 py-2 text-sm font-medium /90 ring-1 ring-white/15 transition hover:bg-white/5"
                >
      ...
      <section className="mx-auto mt-12 max-w-6xl px-4 sm:mt-16">
        <div className="mb-5 flex items-center justify-between">
          ...
          <a href="#alle-undergrupper" className="text-sm /80 hover:">Se alle</a>
        </div>
        ...
        <GlassCard className="p-6 sm:p-8">
          <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm /80">
                Vi har mye utstyr som medlemmer kan leie til en rimelig pris. Her finner du inventar, vilkår og tilgjengelighet.
              </p>
            </div>
    ...
    <header className="sticky top-0 z-20 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/40">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
        ...
        <div className="flex items-center gap-2">
          <a
            href="/dashboard"
            className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-3 py-1.5 text-sm font-medium ring-1 ring-white/15 transition hover:bg-white/15"
          >
    ...
    <footer className="mx-auto mt-16 max-w-6xl px-4 pb-12">
      ...
      <div className="mt-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <p className="text-xs /60">
          © {new Date().getFullYear()} Ålesund Studentsamfunn. Adminside – kun frivillige har tilgang.
        </p>
        <div className="flex items-center gap-4 text-xs /70">
          ...
          <a className="hover:" href="#personvern">Personvern</a>
          <a className="hover:" href="#kontakt">Kontakt</a>
        </div>
      </div>
    </footer>
  );
...
function GlassCard({ className = "", children }: { className?: string; children: React.ReactNode }) {
  return (
    <div
      className={
        "relative rounded-3xl border border-white/10 bg-white/5 shadow-2xl shadow-black/40 backdrop-blur-xl " +
        "[--glass-highlight:linear-gradient(180deg,rgba(255,255,255,0.25),rgba(255,255,255,0))] " +
        "before:pointer-events-none before:absolute before:inset-0 before:rounded-3xl before:bg-[image:var(--glass-highlight)] " +
        className
      }
    >
      {children}
...
function NavBtn({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a
      href={href}
      className="rounded-xl px-3 py-1.5 text-sm /85 ring-1 ring-white/10 transition hover:bg-white/5 hover:"
    >
      {children}
    </a>
  );
...
function QuickLink({ icon: Icon, title, desc }: { icon: React.ComponentType<{ className?: string }>; title: string; desc: string }) {
  return (
    <a
      ...
      className="group flex items-start gap-3 rounded-2xl bg-white/5 p-3 ring-1 ring-white/10 transition hover:bg-white/10"
    >
...
function GroupPill({ icon: Icon, label }: { icon: React.ComponentType<{ className?: string }>; label: string }) {
  return (
    <a
      ...
      className="group inline-flex items-center gap-2 rounded-2xl bg-white/5 px-3 py-2 ring-1 ring-white/10 backdrop-blur-md transition hover:bg-white/10"
    >

...
function NavItem({ item, active, onClick, collapsed }: { item: (typeof navigation)[number]; active: boolean; onClick?: () => void; collapsed?: boolean }) {
  ...
  return (
    <li>
      <Link href={item.href} onClick={onClick} aria-current={active ? "page" : undefined} aria-label={collapsed ? item.name : undefined} title={collapsed ? item.name : undefined} className={cn("group relative flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium outline-none transition", collapsed && "justify-center", active ? "text-foreground" : "text-foreground/80 hover:text-foreground")}>
        {/* Liquid hover ring */}
        <span className={cn("absolute inset-0 rounded-xl", "bg-white/60 dark:bg-white/10 backdrop-blur-md", "opacity-0 ring-1 ring-white/60 dark:ring-white/10 transition-opacity duration-300", active ? "opacity-100" : "group-hover:opacity-100")} />
        <span className={cn("relative z-10 flex items-center", collapsed ? "gap-0" : "gap-3")}>
          <Icon className="h-5 w-5" />
          <span className={cn("overflow-hidden whitespace-nowrap text-left transition-[max-width,opacity,transform] duration-300", collapsed ? "max-w-0 opacity-0 translate-x-1" : "max-w-[200px] opacity-100 translate-x-0")}>{item.name}</span>
        </span>
      </Link>
    </li>
  );

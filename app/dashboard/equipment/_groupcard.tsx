/**
 * Client-side presentation and interaction layer for `equipment/_groupcard.tsx`.
 */
import { Card, CardTitle, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import Link from "next/link";

interface GroupCardProps {
    id: number;
    name: string;
    description: string;
    group_leader: string;
    eq_ammount: number;
}

export default function GroupCard(GroupCardProps: GroupCardProps) {
    return (
        <div>
            <Card className="min-h-40 p-3">
                <CardTitle>{GroupCardProps.name}</CardTitle>
                <CardContent className="mt-12"><Link href={`/dashboard/equipment/by-group/${GroupCardProps.id}`}><Button variant="outline">Se {GroupCardProps.name} sitt utstyr</Button></Link></CardContent>
            </Card>
        </div>
    );
}
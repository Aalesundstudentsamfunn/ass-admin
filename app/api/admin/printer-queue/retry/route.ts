...
/**
 * POST /api/admin/printer-queue/retry
 * Retries one printer queue job via RPC.
 */
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Kunne ikke retrye utskriftsjobben." },
        { status: 400 },
      );
    }
    ...
    if (!row) {
      return NextResponse.json(
        { error: "Kunne ikke hente oppdatert jobb etter retry." },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Ukjent feil ved retry av utskriftsjobb.",
      },
      { status: 500 },
    );
  }
